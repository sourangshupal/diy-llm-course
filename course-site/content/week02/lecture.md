...
**Mermaid diagram of the end-to-end tokenization pipeline:**

```mermaid
flowchart LR
    A[Raw text string] --> B[Normalize\nUnicode / lowercase / strip]
    B --> C[Pre-tokenize\nsplit into candidate units]
    C --> D[Apply learned segmentation\nBPE / WordPiece / Unigram]
    D --> E[Map pieces to integer IDs]
    E --> F[Token ID sequence]
    F --> G[Model: embedding lookup]
    G -.decode.-> H[IDs to pieces]
    H -.decode.-> I[Concatenate pieces to text]
```
