...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model: int, num_heads: int):
        ...
        # Flash Attention can accelerate on GPU
    ...
    def forward(self, x, mask):
        ...
        if self.flash:
            # Flash Attention output is directly the product of Q·K^T and V
            out = torch.nn.functional.scaled_dot_product_attention(
                q, k, v,
                attn_mask=None,
                dropout_p=0,
                is_causal=True   # is_causal=True means an autoregressive causal mask (lower-triangular True matrix)
            )
        else:
...
def generate_with_sampling(
    ...
    # The special token ids 0-4 come from ./bpe_tokenizer/tokenizer.json and must be blocked during generation
    ...
    for _ in range(max_new_tokens):
        ...
        if torch.isnan(probs).any() or torch.isinf(probs).any() or (probs < 0).any():
            # print("[WARN] Invalid probabilities detected, falling back to uniform distribution")
...
if __name__ == "__main__":
    ...
    # Model initialization parameters must match the trained model
    ...
    if os.path.exists("ckpt/epoch_5.pt"):
        ...
        # 4. Configuration consistency check (very important)
        # assert ckpt["config"]["vocab_size"] == model.config.vocab_size
        # assert ckpt["config"]["context_length"] == model.config.context_length

        print(f"[INFO] Successfully loaded checkpoint, using device: {device}")
    else:
        print("[WARNING] Checkpoint file not found, using randomly initialized model")
